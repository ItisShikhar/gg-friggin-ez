...
export async function evaluateWithOpenAi(
    ...
    try {
      ...
      const response = await fetch(openRouterUrl, {
        ...
        headers: {
          ...
          'HTTP-Referer': 'https://github.com/gg-friggin-ez/gg-friggin-ez',
      ...
      if (response.ok) {
        ...
        const calculatedCost =
          promptTokens * INPUT_PRICE_PER_TOKEN + completionTokens * OUTPUT_PRICE_PER_TOKEN;
        ...
        const isProfane = Boolean(parsed.is_profane ?? parsed.contains_profanity);
        ...
        if (parsed.action && ['ALLOW', 'SUSPICIOUS_REVIEW', 'AUTO_CENSOR', 'AUTO_BAN'].includes(parsed.action)) {
          action = parsed.action as ModerationAction;
        } else {
          action = toxicProb >= 0.75 ? 'AUTO_BAN' : toxicProb >= 0.5 ? 'AUTO_CENSOR' : 'ALLOW';
        }

        let outcome: 'correct' | 'wrong' | 'uncertain_routed' | 'unscored' = 'unscored';
  ...
  const costUsd = Number(
    (promptTokens * INPUT_PRICE_PER_TOKEN + completionTokens * OUTPUT_PRICE_PER_TOKEN).toFixed(6)
  );
  const costPerMillionUsd = Number((costUsd * 1_000_000).toFixed(2));
